...
class PendingDeleteRetryJobTest extends TestCase {
	public function testHotJobRetriesYoungRowsEveryFiveMinutes(): void {
		$retry = $this->createMock(PendingBindingService::class);
		$retry->expects($this->once())->method('settleByAge')->with(0, 3600, 200);
	...
	public function testWarmJobRetriesRowsFromFirstDayHourly(): void {
		$retry = $this->createMock(PendingBindingService::class);
		$retry->expects($this->once())->method('settleByAge')->with(3600, 86400, 200);
	...
	public function testColdJobRetriesOlderRowsDaily(): void {
		$retry = $this->createMock(PendingBindingService::class);
		$retry->expects($this->once())->method('settleByAge')->with(86400, null, 200);

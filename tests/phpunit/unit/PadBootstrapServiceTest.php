...
class PadBootstrapServiceTest extends TestCase {
	...
	/**
	 * The pad and the row are the finished half of the job: the file is
	 * still empty, so the next open comes back here and writes what this
	 * one did not. Removing either would trade a working file for a
	 * best-effort remote call that can leave the pad unreachable.
	 */
	public function testAFailedFirstInitKeepsThePadAndRowItAlreadyWrote(): void {
		$fileId = 5150;
		$binding = $this->createMock(BindingService::class);
		$binding->method('findByFileId')->willReturn(null);
		...
		$etherpad->method('createGroup')->willReturn('g.aaaaaaaaaaaaaaaa');
		$etherpad->method('createGroupPad')->willReturn('g.aaaaaaaaaaaaaaaa$nc');
		$etherpad->method('buildPadUrl')->willReturn('https://pad.example.test/p/x');
		// The row names this pad, so nothing is taken away from Etherpad.
		...
		$padFileService = $this->createMock(PadFileService::class);
		$padFileService->method('parseLegacyOwnpadShortcut')->willReturn(null);
		$padFileService->method('buildInitialDocument')->willReturn('---
pad_id: x
---');

		$this->expectException(\RuntimeException::class);

		$this->buildService($binding, $padFileService, $etherpad, $this->buildPadTypePolicy(true), $file)
			->initializeMissingFrontmatter('alice', $file, '');
	}

	/**
	 * The mirror image: no row names this pad, so it is unreachable and the
	 * one thing that must not be left behind.
	 */
	public function testAFailedFirstInitDiscardsAPadNothingPointsAt(): void {
		$fileId = 5151;
		$binding = $this->createMock(BindingService::class);
		$binding->method('findByFileId')->willReturn(null);
		$binding->method('createBinding')
			->willThrowException(new \OCA\EtherpadNextcloud\Exception\BindingException('lost the race'));
		$binding->method('isBoundTo')->willReturn(false);

		$etherpad = $this->createMock(EtherpadClient::class);
		$etherpad->method('buildPadUrl')->willReturn('https://pad.example.test/p/x');
		// A public pad, so the unreachable thing is the pad itself.
		$etherpad->expects($this->once())->method('deletePad');
		...
		$this->expectException(\OCA\EtherpadNextcloud\Exception\BindingException::class);

		$this->buildService($binding, $padFileService, $etherpad, $this->buildPadTypePolicy(false, true), $file)
			->initializeMissingFrontmatter('alice', $file, '');
	}
}

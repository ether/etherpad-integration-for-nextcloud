...
- **Importing protected (group) pads is off by default.** The pad-id inside a legacy `.pad` is written by whoever owns the file, and for a group pad it also picks the Etherpad group a session is later minted for — so opening it grants what that whole group holds. Where this Nextcloud is the only thing writing to its Etherpad server, every group pad already has a binding and a hand-written file collides with it. Where it is not — another Ownpad install, a second Nextcloud, direct use — a foreign group pad has no binding here, and nothing tells it apart from the pads the migration exists for: a genuine Ownpad pad lives in a group Ownpad created, never in one of ours. Switch **Allow importing protected pads from legacy Ownpad files** on in the admin settings for the stretch of a migration, then off again. Public pads are imported either way, and pads that already have a binding are unaffected. A refused import leaves the `.pad` file untouched, so the same open migrates it once the setting is on.

...
- Nextcloud `31.0.9` to `34` (see [appinfo/info.xml](appinfo/info.xml))
...
- **Importing protected (group) pads is off by default.** The pad-id inside a legacy `.pad` is written by whoever owns the file, and for a group pad it also picks the Etherpad group a session is later minted for — so opening it grants what that whole group holds. Turn **Allow importing protected pads from legacy Ownpad files** on only where this Nextcloud is the only thing creating group pads on its Etherpad server. There, a group pad it made carries a binding, so a hand-written file naming one collides with that binding and the access check decides — barring a pad orphaned by a failed cleanup, which the consistency check in the admin settings surfaces. Where anything else writes group pads to the same server — another Ownpad install, a second Nextcloud, direct use — a foreign group pad has no binding here and nothing tells it apart from the pads the migration exists for: a genuine Ownpad pad lives in a group Ownpad created, never in one of ours. Public pads are imported either way, and pads that already have a binding are unaffected. A refused import leaves the `.pad` file untouched, so the same open migrates it once the setting is on.

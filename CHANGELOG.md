...
- **A pad whose text contains `[HTML-BEGIN]` or `[HTML-END]` on a line of its own survives a restore.** The two halves of a snapshot were separated by those marker lines alone, so a pad containing one wrote a file that was byte-for-byte identical to a different pad, and reading it back returned a truncated text. Every snapshot now records the byte length of its text half in the frontmatter and is cut there. Files written before this, and files whose body was edited by hand, keep being read by their markers; the format is unchanged, so an older version of the app reads new files exactly as well as it reads its own. (#256)

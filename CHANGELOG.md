...
- **A pad whose text contains `[HTML-BEGIN]` or `[HTML-END]` on a line of its own survives a restore.** The two halves of a snapshot are separated by those marker lines, and a body could carry no HTML section at all - so a pad holding one of those lines wrote a file byte-for-byte identical to a different pad, and reading it back returned a truncated text. Every snapshot now writes both sections, empty where there is no HTML, which makes the last opening marker always the structural one. A file written before this keeps being read the way it was; an external pad whose stored text was already truncated no longer matches its remote text, so the next sync rewrites it into the current shape. (#256)

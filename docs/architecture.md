...
- `PendingBindingService` settles waiting rows in age buckets (every 5 minutes, then hourly, then daily) and from the admin page, by where the file is now:
  ...
  - in its owner's trash (`pending_delete`): the rest of the trash. The file is recognised in the file cache (`files_trashbin/`) and by node path, found by id without a session, and written on its owner's own storage. Held to the file's `snapshot_rev` like a restore: a pad past it goes into the file (one at it is there already), then the row is deleted, then the pad. A pad that is gone: the row goes, and for a group pad what is left of it (an empty group); a public one takes no further call. A pad that is behind is logged and left in place, and the row goes. Either way the file keeps the snapshot it has.
    ...
    - Etherpad giving no answer, to the question or while the snapshot is read, counts towards the run's stop and leaves the row where it is. An answer without a revision count is no answer.
    - A file that did not get its snapshot waits for the next run, in one log line, `A trashed .pad file did not get its snapshot`, with a `reason` (`TrashSnapshotMiss`). The file's own trouble - unreadable, unparsable, a write refused, an empty file - moves the row to the back and is reported at warning level the first time only; the row's `updated_at` has moved past its `deleted_at` after that. What passes by itself - a lock, a pad that changes while it is read, a file a restore moved - is tried again where the row is, at debug level. A row a trash before 1.1.0 wrote may have the two dates seconds apart and reports at debug level; a row without `deleted_at` reports each time.
    - The file is looked up again right before the write: a restore while the snapshot was fetched has moved it, and a write through the old node would make a new file in the trash. After the write the pad is counted once more, as at trash time.
    - Etherpad is asked with what the run has left once the file is read: a read that took the rest of the run leaves the row where it is, as it does for a restore left undecided.
  ...
  - gone for good, with no `filecache` row left, in either waiting state: pad deleted, then row; a public pad Etherpad already calls absent takes no further call. No restore can come for such a file, and a pad that could not be deleted keeps its row for the next run.
  ...
  - A row is settled by one run at a time. The jobs and the admin page can run at once, and each holds a lock on the row (`etherpad_nextcloud:settle:<fileId>`) while it works on it: two runs finishing the same trash could each write their snapshot, the older one last. A lock that cannot be taken or let go for another reason costs that row, not the run. A process that dies holding one leaves the row skipped until the lock expires, an hour with the database lock. A team folder's trash on the root storage (`__groupfolders/trash/`) is not fetched at all.

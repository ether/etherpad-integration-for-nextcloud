...
- After a click whose button goes away with the focus on it, a second try or a recovery, the embed page hands the focus to the new panel's first action, or to its message, without scrolling. Not on the first load, where it would take the focus from the page it sits in. Its error panel is `role="alert"`, so the first error is read out too.

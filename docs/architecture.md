...
- The viewer and the embed page offer "Try again" where the same open may work later (`isRetryableOpenError()` in `src/lib/pad-open-flow.js`); the button runs the whole open again. That is every answer with `retryable` (`docs/api-reference.md` lists the cases); `pad_file_changed`, which an open meets only while initialising the file, after the server undid its part; and an open that got no answer. `fetchJsonWithTimeout()` marks the last as `unanswered`: its own timeout, a failed network, or a `502`/`503`/`504` without this app's error body (a `message`), from a proxy or Nextcloud in maintenance. It says only that, since whether another try is safe depends on whether the request writes. An initialise that got no answer gets no button: its pad may be set up by now or still being set up, and another open would start a second one. Both clients say "no answer" in a translated sentence of their own rather than the browser's English.
- After a click whose button goes away with the focus on it, a second try or a recovery, the viewer and the embed page hand the focus to the new card's first action, or to its message. Not on the first load. The embed page does it without scrolling, since it sits in another page, and its error panel is `role="alert"`, so the first error is read out too.

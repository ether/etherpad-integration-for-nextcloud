...
- The viewer and the embed page offer "Try again" on every answer with `retryable` (`docs/api-reference.md` lists the cases) and on a request that got no answer in time or failed on the network; the button runs the whole open again. After a second try that fails, the embed page hands the focus to the new button, or to the message; not on the first load, where it would scroll its host page.

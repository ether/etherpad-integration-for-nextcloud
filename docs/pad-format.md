...
- **`snapshot_text_bytes` says where the text half ends.** The markers are ordinary lines, so a pad whose own text contains one produces a body that is byte-for-byte a body with two halves - the markers alone cannot tell those apart. Every snapshot this app writes records the byte length of its text half, and the parser cuts there. It only trusts the length where the markers turn up exactly where it puts them; otherwise it reads the markers instead, which is how a file written before this key and a file whose body was edited by hand are read.
- The marker fallback recognises a section by its terminator and the last opening marker before it, so pad text containing either line on its own keeps as much of it as a delimiter-based reading can. A text-only snapshot whose text both contains `[HTML-BEGIN]` on its own line and ends with `[HTML-END]` stays ambiguous to that fallback and is read as carrying an HTML section - which is why the length is written.
- The key is additive: the format stays `etherpad-nextcloud/1`, and an older version of the app ignores an unknown frontmatter key, so it reads a file written by this version exactly as well as it reads its own.

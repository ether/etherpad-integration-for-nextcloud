...
- `allow_legacy_protected_import` (`yes|no`, default `yes`) — whether a legacy Ownpad `.pad` naming a group pad may still bring that pad in. Switch off where anything other than this Nextcloud creates group pads on the same Etherpad server; see the legacy migration doc.
